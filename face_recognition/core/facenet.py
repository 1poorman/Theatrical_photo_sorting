# import matplotlib.pyplot as plt
import numpy as np
import torch
import torch.nn as nn
import torch.backends.cudnn as cudnn
from facenet.nets.facenet import Facenet as facenet
from facenet.utils.utils import preprocess_input, resize_image, show_config
from config.logConfig import logger
from config.base import CUDA_DEVICE

# from facenet.nets.facenet import ModifiedFacenet as modifiedFacenet

device = torch.device(f"cuda:{CUDA_DEVICE}" if torch.cuda.is_available() else "cpu")
# device = torch.device("cpu")
model_path = "facenet/model_data/facenet_inception_resnetv1.pth"


logger.info("Loading weights into state dict...")
net = facenet(backbone="inception_resnetv1", mode="predict").eval()
net.load_state_dict(torch.load(model_path, map_location=device), strict=False)
logger.info("{} model loaded.".format(model_path))


# --------------------------------------------#
#   使用自己训练好的模型预测需要修改2个参数
#   model_path和backbone需要修改！
# --------------------------------------------#
class Facenet(object):
    _defaults = {
        # --------------------------------------------------------------------------#
        #   使用自己训练好的模型进行预测要修改model_path，指向logs文件夹下的权值文件
        #   训练好后logs文件夹下存在多个权值文件，选择验证集损失较低的即可。
        #   验证集损失较低不代表准确度较高，仅代表该权值在验证集上泛化性能较好。
        # --------------------------------------------------------------------------#
        # "model_path": "facenet/model_data/facenet_inception_resnetv1.pth",
        # --------------------------------------------------------------------------#
        #   输入图片的大小。
        # --------------------------------------------------------------------------#
        "input_shape": [160, 160, 3],
        # --------------------------------------------------------------------------#
        #   所使用到的主干特征提取网络
        # --------------------------------------------------------------------------#
        # "backbone": "inception_resnetv1",
        # -------------------------------------------#
        #   是否进行不失真的resize
        # -------------------------------------------#
        "letterbox_image": True,
        # -------------------------------------------#
        #   是否使用Cuda
        #   没有GPU可以设置成False
        # -------------------------------------------#
        "cuda": 1,
    }

    @classmethod
    def get_defaults(cls, n):
        if n in cls._defaults:
            return cls._defaults[n]
        else:
            return "Unrecognized attribute name '" + n + "'"

    # ---------------------------------------------------#
    #   初始化Facenet
    # ---------------------------------------------------#
    def __init__(self, **kwargs):
        self.__dict__.update(self._defaults)
        for name, value in kwargs.items():
            setattr(self, name, value)
        self.net = net
        # self.generate()

        # show_config(**self._defaults)

        # def generate(self):
        #     # ---------------------------------------------------#
        #     #   载入模型与权值,已更改到全局预加载模型
        #     # ---------------------------------------------------#
        #     print("Loading weights into state dict...")
        #     device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
        #     self.net = facenet(backbone=self.backbone, mode="predict").eval()
        #     self.net.load_state_dict(
        #         torch.load(self.model_path, map_location=device), strict=False
        #     )
        #     print("{} model loaded.".format(self.model_path))

        # ---------------------------------------------------#
        # 额外添加的全连接层，改变输出维度，但会导致效果下降
        # ---------------------------------------------------#
        # modified_model = modifiedFacenet(self.net, new_embedding_size=512).eval()
        # self.net = modified_model

        if self.cuda:
            self.net = torch.nn.DataParallel(self.net)
            cudnn.benchmark = True
            self.net = self.net.cuda()

    def output_feature(self, image):
        with torch.no_grad():
            image_1 = resize_image(
                image,
                [self.input_shape[1], self.input_shape[0]],
                letterbox_image=self.letterbox_image,
            )
            photo = torch.from_numpy(
                np.expand_dims(
                    np.transpose(
                        preprocess_input(np.array(image_1, np.float32)), (2, 0, 1)
                    ),
                    0,
                )
            )
            if self.cuda:
                photo = photo.cuda()
            output = self.net(photo).cpu()
            return output
