...
| M15 | `review_queue.py` + API | 🟢 离线完成 | 复核队列、接受/修改/拒绝、不可变修订日志、最终标签导出；审核 UI 待做 |
...
/home/huachenghao/.conda/envs/face_scrfd_arcface/bin/python tests/acceptance_multimodal_m16.py  # 8/8 通过
```
...
5. ✅ M15 复核核心与 API 通过（`tests/test_scene_review_api.py`）。
6. ✅ M16 离线验收 8/8 通过（`tests/acceptance_multimodal_m16.py`），含端到端串联。
7. 下一步：审核 UI 页面；用真实 SigLIP2 `ImageEmbedder` 与 `.env` 本地端点联调；在目标机跑 gated 重型回归并归档 `outputs/acceptance/m16/`。
